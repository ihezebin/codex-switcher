...
contextBridge.exposeInMainWorld('codexAPI', {
  ...
  prepareSessions: () => ipcRenderer.invoke('codex:prepare-sessions'),
  listSessions: (allowStale = false) => ipcRenderer.invoke('codex:list-sessions', allowStale),
  getSession: (file) => ipcRenderer.invoke('codex:get-session', file),
  deleteSession: (file) => ipcRenderer.invoke('codex:delete-session', file),
  resumeSession: (file) => ipcRenderer.invoke('codex:resume-session', file),
  getUsageStatistics: (range) => ipcRenderer.invoke('codex:usage-statistics', range),
