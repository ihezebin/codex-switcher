...
- **多配置管理**：创建、编辑、重命名、删除和快速切换 Codex Profile，自动识别并标记当前已应用的配置。
- **可视化配置**：通过表单维护 Model、Review Model、Base URL 和 API Key，也可以直接编辑完整 TOML 配置。
- **模型与连接检测**：从已配置的服务加载模型列表，测试实际 API 连接，并展示 HTTP 状态码和格式化响应体。
- **托盘快速切换**：在 macOS 菜单栏或 Windows 通知区域直接切换 Profile，关闭主窗口后仍可在后台运行。
- **会话管理**：查看 Codex 会话列表、项目目录、会话详情和完整对话记录，支持删除会话以及在当前系统终端中恢复会话。
- **用量统计**：按当天、7 天或 1 个月查看请求数、输入/输出 Tokens、缓存读取 Tokens 和估算成本，并提供趋势曲线、模型统计和请求日志。
- **会话扫描缓存**：会话和用量数据使用本地 SQLite 缓存，未变化的历史会话无需重复解析，提升再次打开时的加载速度。
- **应用更新**：自动检测新版本、展示更新说明、下载安装包，并支持通过 `minimumVersion` 要求旧版本强制升级。
- **界面个性化**：支持亮色/暗色主题、中英文界面，以及小赖字体、霞鹜文楷和朱雀仿宋三种界面字体。
...
`proxyBaseUrl` 用于 GitHub release 下载代理。默认值是 `https://ghfast.top/`，配置后会先尝试代理地址，例如 `https://ghfast.top/https://github.com/...`，代理失败后再回退原始 `downloads` 地址。如果不想使用代理，可以把 `proxyBaseUrl` 配成空字符串。
